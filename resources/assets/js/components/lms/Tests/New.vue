<template>
    <div>
        <div class="row">
                ...
                <form class="card card-outline-primary mb-4">
                    <div class="card-block">
                        <div class="form-group">
                            <label class="small text-muted" style="font-weight:500" for="field1">название тестирования</label>
                            <input id="field1" type="text" class="form-control input-primary" v-model="test.name" placeholder="English File 3e Preint Quicktest 02">
                            <form-error v-if="errors && errors['tests.'+index+'.name']" :errors="errors">
                                {{ errors['tests.'+index+'.name'][0] }}
                            </form-error>
                        </div>
                        <div class="form-group">
                            <label class="small text-muted" style="font-weight:500" for="field3">ограничение времени (минут)</label>
                            <input id="field3" type="text" class="form-control" v-model="test.duration" placeholder="12">
                        </div>
                        <div class="form-group">
                            <label class="small text-muted" style="font-weight:500" for="field4">открыть доступ</label>
                            <select id="field4" v-model="test.access" class="form-control">
                                <option value="0">во время занятия</option>
                                <option value="1">после занятия</option>
                            </select>
                        </div>
                    </div>
                </form>
            ...
            <div class="col-9">
                <div class="card mb-5" v-for="(task_group,index2) in test.task_groups">
                    ...
                    <div class="card-block">
                        <div v-for="(task,index3) in task_group.tasks">
                            <div class="mb-2 clearfix">
                                <small class="badge badge-default" style="font-weight:500">Задание #{{ index3+1 }}</small>
                            ...
                            <div class="form-group mb-4" enctype="multipart/form-data">
                                ...
                                <form-error style="display:inline-block;" v-if="errors && errors['tests'+index+'.task_groups.'+index2+'.tasks.'+index3+'.description']" :errors="errors">
                                    {{ errors['tests'+index+'.task_groups.'+index2+'.tasks.'+index3+'.description'][0] }}
                                </form-error>
                                ...
                                <input ref="fileInput" :id="index3+'taskImage'+index2" type="file" style="display:none" accept="image/*" v-on:change="uploadTaskImage($event,task)"/>
                                <button @click="emitClickTaskImage(index3,index2)" class="btn btn-sm btn-secondary mt-2">
                                ...
                                <input ref="fileInput" :id="index3+'taskAudio'+index2" type="file" style="display:none" accept="audio/*" v-on:change="uploadTaskAudio($event,task)"/>
                                <button @click="emitClickTaskAudio(index3,index2)" class="btn btn-sm btn-secondary mt-2">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group mt-1" style="flex-wrap: wrap;" v-for="(question,index4) in task.questions">
                                <span class="input-group-addon"  v-if="task.task_type_id == 3">
                                    ...
                                    <input :id="'input_'+index4+'_'+index3+'_'+index2+'_'+index" @keyup.enter="onEnter(task,index4,index3,index2,index)" type="text" class="form-control" :class="{'form-control-sm': task.task_type_id != 5}" v-model="question.content">
                                    ...
                                    <form-error style="width:100%;" v-if="errors && errors['tests.'+index+'.task_groups.'+index2+'.tasks.'+index3+'.questions.'+index4+'.content']" :errors="errors">
                                        {{ errors['tests.'+index+'.task_groups.'+index2+'.tasks.'+index3+'.questions.'+index4+'.content'][0] }}
                                    </form-error>
                                </div>
                            ...
                            <div v-if="task.task_type_id == 6" class="form-group">
                                ...
                                <form-error class="d-block" v-if="errors && errors['tests.'+index+'.task_groups.'+index2+'.tasks.'+index3+'.words']" :errors="errors">
                                    {{ errors['tests.'+index+'.task_groups.'+index2+'.tasks.'+index3+'.words'][0] }}
                                </form-error>
                            </div>
        ...
        <div class="my-5 text-right">
                <span @click="$emit('cancel')" class="btn btn-outline-secondary">
                    Выйти
                </span>
            <span @click="$emit('save')" class="btn btn-primary">
                    Сохранить и выйти
                </span>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            addTaskGroup() {
                this.test.task_groups.push({id: '', tasks: [], section_id: 1});
            },
            getSectionName(id) {
                var name = '';
            ...
            removeTaskGroup(task_group) {
                var index = this.test.task_groups.indexOf(task_group);
                if (index > -1) this.test.task_groups.splice(index, 1);
            },
            ...
            uploadTaskImage(event,task) {
                let _this = this;
                var formData = new FormData();
                formData.append('file',event.target.files[0]);
                this.$nextTick(function () {
                    postFile(_this, '/api/upload-file', formData, function(progress) {
                        task.uploadImage = progress;
                    }, function (response) {
                        task.uploadImage = 0;
                        task.image = response.data
                    });
                });
            },
            uploadTaskAudio(event,task) {
                let _this = this;
                var formData = new FormData();
                formData.append('file',event.target.files[0]);
                this.$nextTick(function () {
                    postFile(_this, '/api/upload-file', formData, function(progress) {
                        task.uploadAudio = progress;
                    }, function (response) {
                        task.uploadAudio = 0;
                        task.audio = response.data
                    });
                });
            },
            emitClickTaskImage(index,index2) {
                $('#'+index+'taskImage'+index2).click();
            },
            emitClickTaskAudio(index, index2) {
                $('#'+index+'taskAudio'+index2).click();
            },
            addQuestion(task) {
            ...
            onEnter(task,index,temp,temp2,temp3) {
                if(task.task_type_id != 5) {
                    task.questions.push({id: '', content: '', is_correct: 0});
                    this.$nextTick(function() {
                        let i = task.questions.length - 1;
                        $('#input_'+i+"_"+temp+'_'+temp2+'_'+temp3).focus();
                    });
                }
            },
        }
