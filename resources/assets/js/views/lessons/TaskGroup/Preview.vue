<template>
    <div class="card mb-5">
        <div class="card-block">
            <b-carousel ref="carousel" :interval="0" v-model="slide">
                <b-carousel-slide v-for="task in tasks" :key="task.id" :contentTag="'span'">
                    <div class="w-100">
                        <div class="h4 mb-3">
                            {{ task.description }}
                        ...
                        <div v-for="question in task.questions" v-if="task.task_type_id==1">
                            ...
                            <span v-for="(word,index) in $lms.taskQuestions(question.content)">
                                ...
                                <span v-else draggable="true" @dragend="question['answer'+index]='?'" @dragstart="selected=question['answer'+index]" @dragover.prevent @drop="onDrop(question,index)" class="badge"  :class="{'badge-default pl-3 pr-3 ml-2 mr-2': question['answer'+index]=='?', 'badge-primary mr-1 ml-1': question['answer'+index]!='?'}">
                        ...
                        <div v-for="question in task.questions" v-if="task.task_type_id==5">
                            ...
                            <hr>
                            <span v-for="(word,index) in $lms.taskQuestions(question.content)">
                                <span draggable="true" @dragend="question['answer'+index]='?'" @dragstart="selected=question['answer'+index]" @dragover.prevent @drop="onDrop(question,index)" class="badge"  :class="{'badge-default pl-3 pr-3 ml-2 mr-2': question['answer'+index]=='?', 'badge-primary mr-1 ml-1': question['answer'+index]!='?'}">
                                    <span class="fa mr-2 ml-2 fa-ellipsis-v" v-if="question['answer'+index]!='?'"></span>
                                    {{ question['answer'+index] }}
                                </span>
                            </span>
...
<script>
    ...
    export default {
        ...
        methods: {
            moveTo(value) {
                this.$refs.carousel.index = value;
                this.slide = value;
            },
            pressed(value) {
                return value==this.slide ? true : false;
            },
